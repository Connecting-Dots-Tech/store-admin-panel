...
let storesDB = mockApi.components.examples.ecommerce_store.value;
...
mock.onGet('/api/ecommerce/stores').reply((config) => {
  return [200, storesDB];
});

mock.onPost('/api/ecommerce/stores').reply(({ data }) => {
  const newStore = { id: FuseUtils.generateGUID(), ...JSON.parse(data) };
...
mock.onDelete('/api/ecommerce/stores').reply(({ data }) => {
  const ids = JSON.parse(data);
  storesDB = storesDB.filter((item) => ids.includes(item.id));

  return [200, storesDB];
});

mock.onGet(/\/api\/ecommerce\/stores\/[^/]+/).reply(({ url, data }) => {
  const { id } = url.match(/\/api\/ecommerce\/stores\/(?<id>[^/]+)/).groups;

  return [200, _.find(storesDB, { id })];
});

mock.onPut(/\/api\/ecommerce\/stores\/[^/]+/).reply(({ url, data }) => {
  const { id } = url.match(/\/api\/ecommerce\/stores\/(?<id>[^/]+)/).groups;

  _.assign(_.find(storesDB, { id }), JSON.parse(data));

  return [200, _.find(storesDB, { id })];
});

mock.onDelete(/\/api\/ecommerce\/stores\/[^/]+/).reply((config) => {
  const { id } = config.url.match(/\/api\/ecommerce\/stores\/(?<id>[^/]+)/).groups;

  _.remove(storesDB, { id });
